...
public class AppNoteServer {
    ...
    private void AppNoteStart(String[] args) {
        try {
            ...
            for(int i = 0; i < args.length; ++i) {
                logger.log(Level.FINE, "param " + args[i]);
            }

            logger.log(Level.FINE, "Started AppNoteServer ......" + args.length + "  NO2 = " + args);
            PathwayServiceProvider server = PathwayServiceProvider.newPathwayServiceProvider(new AppNoteServer(), new MessageHandler(), 32767, tandemErrorMap);
            server.setServiceNameOffset(0);
            server.setServiceNameLength(0);
            server.start();
    ...
    public IO_Appnotesrv_rep GENSERVICE(IO_Appnotesrv_req iMessageIn) {
        ...
        AppnoteDao appnoteDao = new AppnoteDaoImpl(jdbcTemplate);
